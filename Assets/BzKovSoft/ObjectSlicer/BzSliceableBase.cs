...
#pragma warning restore 0649
		...
		{
            _defaultSliceMaterial = GameController.currentSkin.hopheyCapMat;
			MeshRenderer[] renderers = GetRenderers(gameObject);
            //Material[] renderMaterials = { GetCapMaterial(gameObject), GetBodyMaterial(gameObject) };
			...
			for (int i = 0; i < renderers.Length; i++)
			{
                ...
                //var renderMat = renderMaterials[i];
				...
				var meshDissector = new BzMeshDataDissector(mesh, sliceTryData.plane, renderer.materials, adapter, configuration);
        ...
        protected virtual void GetNewObjects(out GameObject resultObjNeg, out GameObject resultObjPos)
        {
            ...
            Rigidbody rb = resultObjNeg.GetComponent<Rigidbody>();
            rb.isKinematic = false;
            rb.useGravity = true;
            resultObjNeg.tag = "cutted";
            resultObjNeg.GetComponent<ObjectMover>().isCut = true;
            resultObjNeg.GetComponent<ObjectMover>().stopTween();
            ObjectMover.cutTrigger = true;


            resultObjPos.name = resultObjNeg.name;
			resultObjNeg.name = "cutted " + resultObjNeg.name;
            ObjectMover.sliceCount++;
        }
		...
		private MeshRenderer[] GetRenderers(GameObject gameObject)
		{
            MeshRenderer[] render = gameObject.GetComponentsInChildren<MeshRenderer>();

            foreach (MeshRenderer meshRenderer in render)
            {
                meshRenderer.materials[0] = GameController.currentSkin.hopheyCapMat;
                meshRenderer.materials[1] = GameController.currentSkin.hopheyBodyMat;
            }

            return render;
		}

        private Material GetBodyMaterial(GameObject gameObject)
        {
            return GameController.currentSkin.hopheyBodyMat;
        }

        private Material GetCapMaterial(GameObject gameObject)
        {
            return GameController.currentSkin.hopheyCapMat;
        }

        private Mesh GetMeshFilter(GameObject gameObject)
        {
            return GameController.currentSkin.hopheyMesh;
        }
